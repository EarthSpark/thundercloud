...
from sparkmeter.tests.test_data_factory import GroundFactory, MeterFactory, OperatorFactory, TariffFactory
...
class DashboardViewTest(WebViewTestCaseBase):
    ...
    def test_last_two_months_energy_purchase(self, client):
        ...
        response = client.get(path)
        d = [
            {"val": 8.0, "col": "tariff 1", "idx": "Feb, 2016"},  # 3 + 5 = 8
            {"val": 6.0, "col": "tariff 2", "idx": "Feb, 2016"},  # 1 + 5 = 6
            {"val": 7.0, "col": "tariff 1", "idx": "Mar, 2016"},  # 7 = 7
            {"val": 0.0, "col": "tariff 2", "idx": "Mar, 2016"},
        ]
    ...
    def test_last_two_months_monthly_consumption(self, client):
        ...
        response = client.get(path)
        d = [
            {"val": 10.0, "col": "tariff 1", "idx": "Feb, 2016"},  # 4 + 6 = 10
            {"val": 8.0, "col": "tariff 2", "idx": "Feb, 2016"},  # 2 + 6 = 8
            {"val": 8.0, "col": "tariff 1", "idx": "Mar, 2016"},  # 8 = 8
            {"val": 0.0, "col": "tariff 2", "idx": "Mar, 2016"},
        ]
        assert d == response.json()["data"][0]["values"]
        self.verify_response(response)
    ...
    def test_last_two_months_daily_avg_consumption(self, client):
        ...
        d = [
            {
                "val": pytest.approx(0.344827586206897),  # (4 + 6) / 29
                "col": "tariff 1",
                "idx": "Feb, 2016",
            },
            # (2 + 6) / 31 = 0.27..
            {"val": pytest.approx(0.275862068965517), "col": "tariff 2", "idx": "Feb, 2016"},
            {"val": 8.0, "col": "tariff 1", "idx": "Mar, 2016"},  # 8 / 1 = 8
            {"val": 0.0, "col": "tariff 2", "idx": "Mar, 2016"},
        ]
        assert d == response.json()["data"][0]["values"]
        self.verify_response(response)
        ...
        for params in [
            ...
            for user in users:
                ...
                if params.get("SERIAL"):
                    path += "?" + urllib.parse.urlencode(dict(ground_serial=params.get("SERIAL")))
                response = client.get(path)
                variant = "%s-%s" % (where, user.username)
                self.verify_response(response, variant=variant)
