...
from sparkmeter.tests.test_data_factory import (EventFactory, MeterFactory, ReadingFactory,
...
class DashboardDailyTariffSummaryTest(SparkMeterTestCaseBase):
    ...
    def test_get_items_since_view(self):
        # All pre-paid
        meter = MeterFactory(config__state=MeterConfig.STATE_AUTO,
                             ground=self.ground)
        ...
        # Not Prepaid
        tariff = TariffFactory()
        meter = MeterFactory(ground=self.ground,
                             tariff=tariff,
        ...
        query = DashboardDailyTariffSummary.get_items_since_view(
            date, ground=self.ground, prepaid_only=False)
        results = self.session.execute(query)
        assert list(results) == [
            (u'tar\xefff01', datetime.date(2013, 1, 2), 0, 0, 0.0, 0),
            (u'tar\xefff02', datetime.date(2013, 1, 2), 0, 0, 0.0, 0),
        ]
    ...
    def test_get_monthly_summary_view(self):
        ...
        expected = [
            # Tariff 1 February
            (u'tariff 1', datetime.date(2016, 2, 1),
             8,    # 3 + 5
             10.0,   # 4 + 6
             0.344827586206897),   # (4 + 6) / 29
            # Tariff 1 March
            (u'tariff 1', datetime.date(2016, 3, 1),
             7,   # 7
             8.0,   # 8
             8.0),   # 8 / 1
            # Tariff 2 February
            (u'tariff 2', datetime.date(2016, 2, 1),
             6,    # 1 + 5
             8.0,    # 2 + 6
             0.275862068965517),   # (2 + 6) / 29
        ]
