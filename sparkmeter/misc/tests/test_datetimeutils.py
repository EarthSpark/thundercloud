...
from sparkmeter.misc.datetimeutils import (datetime_interval_to_day_interval, get_next_clear,
                                           get_next_heartbeat, month_delta, reset_datetime_to_time)
...
def test_datetime_interval_to_day_interval_same_day():
    intervals = datetime_interval_to_day_interval(
        ...
        datetime.datetime(2001, 1, 5, 18, 0))
...
def test_datetime_interval_to_day_interval_one_crossing():
    intervals = datetime_interval_to_day_interval(
        datetime.datetime(2001, 1, 5, 23, 0),
        datetime.datetime(2001, 1, 6, 1, 0))
...
def test_datetime_interval_to_day_interval_start_at_midnight():
    intervals = datetime_interval_to_day_interval(
        ...
        datetime.datetime(2001, 1, 6, 10, 0))
...
def test_datetime_interval_to_day_interval_errors():
    ...
    with pytest.raises(ValueError, match=msg):
        datetime_interval_to_day_interval(start=now.replace(
            year=3000, month=1, day=1), end=now)
            ...
            for m in [4, 30, 44, 59]:
                for s in [0, 30]:
                    ...
                    with freezegun.freeze_time(dt):
                        expected = dt.replace(minute=dt.minute // period * period,
                                              second=0,
                                              microsecond=0) + datetime.timedelta(minutes=period)
...
@pytest.mark.parametrize('dt_tuple, months, expected', (
