...
def get_x_variant_name(model_number):
    ...
    if not series_match:
        raise MeterError(
            MeterError.UNKNOWN_MODEL, "Model number {} is not a SparkMeter model".format(model_number)
        )
...
def rekey_serial(serial, new_model):
    ...
    if not match:
        raise MeterError(MeterError.INVALID_SERIAL, "serial {} is not a valid meter serial".format(serial))
            ...
            if local_current_limits[model["name"]] > model["inrush_limit"]:
                if is_x_variant(model["name"]):
                    ...
                    try:
                        ...
                        logger.warning(
                            'Creating new meter variant for "%s": %s', model["name"], json_dumps(variant)
                        )
