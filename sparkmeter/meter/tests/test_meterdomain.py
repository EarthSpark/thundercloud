...
class MeterTest(SparkMeterTestCaseBase):
    def test_get_all_customer_meters(self):
        ...
        meter = MeterFactory(code=1, config__state=MeterState.STATE_ON.id, tariff=tariff)
    ...
    def test_get_all_totalizer_meters(self):
        tariff = TariffFactory(flat_load_limit=50)
        MeterFactory(code=1, config__state=MeterState.STATE_ON.id, tariff=tariff)
    ...
    def test_needs_update(self, logger):
        ...
        # no update needed
        assert not meter._needs_update(current_state=1, current_load_limit=50, override_meter_state=False)

        # updates needed
        assert meter._needs_update(current_state=None, current_load_limit=None, override_meter_state=False)
        assert meter._needs_update(
            current_state=MeterState.STATE_OFF.id, current_load_limit=50, override_meter_state=False
        )
        assert meter._needs_update(
            current_state=MeterState.STATE_ON.id, current_load_limit=60, override_meter_state=False
        )
        ...
        logger.check(
            (
                ...
                "INFO",
                "Meter SM15R-01-00000001 has state and load limit are up to date, skipping update.",
            ),
            (
                "sparkmeter.meter.meterdomain",
                "INFO",
            ...
            (
                "sparkmeter.meter.meterdomain",
                "INFO",
                "Meter SM15R-01-00000001 has a current state of Off, but it should be On, requesting update.",
            ),
            (
                "sparkmeter.meter.meterdomain",
                "INFO",
                "Meter SM15R-01-00000001 has a load limit of 60, but it should be 50, requesting update.",
            ),
        )

        logger.clear()
        assert meter._needs_update(current_state=None, current_load_limit=None, override_meter_state=True)
        assert meter._needs_update(
            current_state=MeterState.STATE_ON.id, current_load_limit=60, override_meter_state=True
        )
        ...
        logger.check(
            ...
            (
                "sparkmeter.meter.meterdomain",
                "INFO",
                "Meter SM15R-01-00000001 has a current state of On, "
                "but it should be Off (override enabled), requesting update.",
            ),
        )

        # Power limits changes are ignored when override is enabled
        assert not meter._needs_update(
            current_state=MeterState.STATE_OFF.id, current_load_limit=50, override_meter_state=True
        )
    ...
    def test_needs_update_force(self, logger):
        ...
        logger.check(
            (
                "sparkmeter.meter.meterdomain",
                "INFO",
                "Meter SM15R-01-00000001 is requesting update by force (load_limit=unset, state=unset).",
            ),
        )
    ...
    def test_current_state_text(self):
        meter = MeterFactory()
        assert meter.current_state_text == "On"

        meter.system_info.current_state = MeterState.STATE_THROTTLE.id
        assert meter.current_state_text == "Throttle"

        meter.system_info.current_state = 99
        assert meter.current_state_text == "Unknown"
    ...
    def test_check_can_sell_from_api(self, api_role):
        ...
        assert str(exc_info.value) == (
            "user 'testüser-001' cannot repay debt for meter 'SM15R-01-00000001': "
            "api users cannot repay debt."
        )
    ...
    def test_check_can_sell_from_ground(self, config):
        ...
        assert str(exc_info.value) == (
            "user 'testüser-001' cannot repay debt for meter 'SM15R-01-00000001': "
            "transactions for this meter can only be placed on ground 'test micrøgrid 1'."
        )
    ...
    def test_check_can_sell_from_user_permission(self):
        ...
        assert str(exc_info.value) == (
            "user 'testüser-001' cannot repay debt for meter 'SM15R-01-00000001': "
            "user is not associated with ground 'test micrøgrid 1'."
        )
    ...
    def test_check_can_sell_to_user_permission(self):
        ...
        with pytest.raises(TransactionError) as exc_info:
            meter.check_can_sell_to(user)
        assert str(exc_info.value) == (
            "user 'testüser-001' cannot buy credit for meter 'SM15R-01-00000001': "
            "user is not associated with ground 'test micrøgrid 1'."
        )
    ...
    def test_maybe_convert_negative_balance_to_debt(self, mocker):
        ...
        assert event_create.mock_calls == [
            mock.call("config-parameter-changed", obj=ParameterObject.ALLOW_NEGATIVE_BALANCE.parameter),
            mock.call("config-parameter-changed", obj=ParameterObject.ALLOW_NEGATIVE_BALANCE.parameter),
            mock.call("config-parameter-changed", obj=ParameterObject.ALLOW_NEGATIVE_BALANCE.parameter),
        ]
...
class MeterSystemInfoTest(SparkMeterTestCaseBase):
    def test_update_from_set_config(self):
        ...
        system_info.update_from_set_config(
            command="enable", application_version="app-ver", bootloader_version="boot-ver", power_limit=100
        )
