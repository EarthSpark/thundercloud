...
class MeterViewTest(WebViewTestCaseBase):
    ...
    def test_unknown_server_error(self, client, MeterView):
        MeterView.validate_serial.side_effect = MeterError("unknown", "Unknown message")
        path = "/meter/add-meter/totalizer"
        data = {
            "subnet": 1,
            "serial": "SM15R-01-0000007B",
    ...
    def test_view_cloud(self, client, config):
        meter = MeterFactory(system_info__last_config_datetime=datetime.datetime(2010, 1, 1))
        self.session.commit()
    ...
    def test_chart_data(self, client):
        ...
        for reading in range(3):
            ...
            ReadingFactory(heartbeat_start=start, heartbeat_end=end, _meter=None, meter=str(meter.code))
        self.session.commit()
    ...
    def test_chart_data_group_by(self, client):
        ...
        for reading in range(3):
            ...
            ReadingFactory(
                heartbeat_start=start, heartbeat_end=end, energy=reading * 10, meter=str(meter.code)
            )
        self.session.commit()
    ...
    def test_edit_with_tags(self, client, config):
        tariff = TariffFactory()
        meter = MeterFactory(billing__tariff=tariff)
        ...
        assert ["existing-tag"] == [t.name for t in meter.tags]
        ...
        assert sorted(["existing-tag", "new-tag"]) == sorted([t.name for t in meter.tags])
        ...
        assert sorted(["new-tag"]) == sorted([t.name for t in meter.tags])
        assert MetersTags.query.count() == 2
        assert MeterTag.query.count() == 2
    ...
    def test_messages_json(self, client, mocker):
        ...
        SMSMessageFactory(phone_number=meter.customer.phone_number, direction=SMSMessage.DIRECTION_IN)
        SMSMessageFactory(
            phone_number=meter.customer.phone_number, event=event, direction=SMSMessage.DIRECTION_OUT
        )
        self.session.commit()
    ...
    def test_meters_filtering(self, client, config, operator_role, vendor_role):
        ...
        other = GroundFactory()
        self.session.commit()
        MeterFactory(serial="SM15R-01-00000001", ground=self.ground)
        MeterFactory(serial="SM15R-01-00000002", ground=other)
        o1 = OperatorFactory(roles=[operator_role], username="operator-only-1", grounds=[self.ground])
        o2 = OperatorFactory(roles=[operator_role], username="operator-only-2", grounds=[other])
        o3 = OperatorFactory(roles=[operator_role], username="operator-all", grounds=[self.ground, other])
        v1 = VendorFactory(roles=[vendor_role], username="vendor-only-1", grounds=[self.ground])
        v2 = VendorFactory(roles=[vendor_role], username="vendor-only-2", grounds=[other])
        v3 = VendorFactory(roles=[vendor_role], username="vendor-all", grounds=[self.ground, other])
        self.session.commit()
