...
from sparkmeter.meter.meterutils import (ModelMapper, get_x_variant_name, is_x_variant,
                                         merge_local_meter_models, rekey_serial)
...
@pytest.mark.parametrize('serial,model,expected', (('SM5R-01-22200011', 'SM5XR', 'SM5XR-01-22200011'),
...
class TestMergeLocalMeterModels(object):
    ...
    def test_all_inrush_outside_limits(self, sample_meter_models):
        ...
        with LogCapture('sparkmeter.meter.meterutils', level=logging.WARNING) as cap:
            ...
            for log, expected in zip(cap.records, ['Raising', 'Creating', 'Raising', 'Creating', 'Creating',
                                                   'Creating', 'Creating']):
        ...
        for local_model in merged:  # iterate over the resultant model set
            ...
            if is_x_variant(local_name):  # if this is an X variant...
                if local_name in sample_dict:  # if it existed, verify its inrush was updated
                    ...
                    assert sample_dict[local_name.replace('X', '')]['inrush_limit'] + 1 == (
                        local_model['inrush_limit']), json.dumps(local_model)
            else:  # if this isn't an X variant, verify the inrush wasn't changed
                assert sample_dict[local_name]['inrush_limit'] == local_model['inrush_limit'], (
                    json.dumps(local_model))
