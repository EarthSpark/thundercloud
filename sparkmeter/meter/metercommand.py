...
def remove(serial):
    ...
    for trans, _ in meter.get_transaction_view():
        logger.warning("Transaction %s %s %s %s" % (trans.id, trans.created, trans.amount, trans.acct_type))
...
def get_heartbeat_reading(mac):
    ...
    app = getUtility(IApplication)
    app.setup_databases()
    reading = Reading.query.filter_by(meter=mac).order_by(Reading.heartbeat_end.desc()).first()
    if reading is None:
