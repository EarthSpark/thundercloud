...
from sparkmeter.transaction.transactionview import parse_datatables_args as parse_datatables_transaction_args
...
def meters_json():
    ...
    meter_views = MeterView.get_view(meter_type=meter_type, ground=ground, user=get_current_user())
    return jsonify(meters=format_meter_view(meter_views))


@meter.route("/meter/add-meter/<string:meter_type>", methods=["GET", "POST"])
@meter.route("/meter/add-meter", methods=["GET", "POST"])
...
def add(meter_type=Meter.TYPE_CUSTOMER):
    ...
    return form.render(meter_type=meter_type, ground=ground)


@meter.route("/meter/<string:meter_serial>/edit", methods=["GET", "POST"])
...
def edit(meter_serial, phase=None):
    ...
    return form.render(meter=meter, meter_type=meter.meter_type, ground=meter.ground)
...
def view(meter_serial):
    ...
    has_accounts = (
        SalesAccount.get_accounts_by_user_ground(
            user,
            meter.ground,
        ).count()
        > 0
    ...
    load_limit_capped = (
        meter.is_customer_meter()
        and meter_power_limit < meter.tariff.get_current_load_limit()
        and (meter_power_limit == meter.system_info.current_user_power_limit)
    )
...
@meter.route("/meter/<string:meter_serial>/set-state", methods=["POST"])
...
def set_state(meter_serial):
    ...
    return jsonify(state_value=meter.state_value, state_text=str(meter.state_text))


@meter.route("/meter/<string:meter_serial>/verify-phone-number", methods=["PUT"])
...
def chart(meter_serial):
    ...
    if meter.is_totalizer_meter():
        exclude_fields.extend(["cost", "acct_credit", "acct_plan", "acct_debt", "rate", "tou_modifier"])
    ...
    if not form.start.data:
        form.start.data = (datetime.datetime.utcnow().date() - datetime.timedelta(days=5)).strftime(
            "%Y/%m/%d"
        )
