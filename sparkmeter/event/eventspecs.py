...
from sparkmeter.event.eventkeywords import (BooleanKeyword, CurrencyKeyword, DateTimeKeyword,
                                            EnergyKeyword, StringKeyword)

logger = logging.getLogger()
KEYWORD_CUSTOMER_NAME = StringKeyword(
    'customer_name',
    _('Name of the customer.'),
    _('John Doe'))

KEYWORD_CUSTOMER_CODE = StringKeyword(
    'customer_code',
    _('Code of the customer.'),
    _('CC1618'))

KEYWORD_CREDITS_BALANCE = CurrencyKeyword(
    'credits_balance',
    _('Credit balance for the customer.'),
    10.34)

KEYWORD_DEBT_BALANCE = CurrencyKeyword(
    'debt_balance',
    _('Debt balance for the customer.'),
    42.91)

KEYWORD_CURRENT_TARIFF_NAME = StringKeyword(
    'current_tariff_name',
    _('Name of Tariff used by the meter.'),
    _('ET60'))


KEYWORD_SERIAL = StringKeyword(
    'serial',
    _('Meter serial number.'),
    _('SM15R-01-00DDBA11'))


KEYWORD_IS_RUNNING_PLAN = BooleanKeyword(
    'is_running_plan',
    _('If the tariff used includes a plan, has it been paid? (yes/no)'),
    True)

KEYWORD_PLAN_BALANCE = CurrencyKeyword(
    'plan_balance',
    _('For tariffs with plans, the Plan balance.'),
    17.26)

KEYWORD_LAST_ENERGY = EnergyKeyword(
    'last_energy',
    _('Last energy value read for the meter, in kWh.'),
    8.663)

KEYWORD_OPERATING_MODE = StringKeyword(
    'operating_mode',
    _('Operating mode of the meter (On/Off/Auto).'),
    _('Auto'))

KEYWORD_TOTAL_CYCLE_ENERGY = EnergyKeyword(
    'total_cycle_energy',
    _('Total amount of energy used since the beginning of the month, in kWh.'),
    2.804)

KEYWORD_TRANSACTION_AMOUNT = CurrencyKeyword(
    'amount',
    _('Amount of the placed transaction.'),
    25)

KEYWORD_TRANSACTION_VENDOR = StringKeyword(
    'vendor',
    _('Name of vendor used for the placed transaction.'),
    _('Jane Doe'))

KEYWORD_TRANSACTION_CREATION_DATETIME = DateTimeKeyword(
    'creation_datetime',
    _('Time when the transaction was placed.'),
    datetime.datetime(2016, 3, 31, 16, 1, 0))

KEYWORD_TRANSACTION_SOURCE = StringKeyword(
    'source',
    _('Source of the placed transaction (cash/bonus).'),
    _('cash'))

KEYWORD_TRANSACTION_STATUS = StringKeyword(
    'status',
    _('Status of placed transaction '
      '(not processed/processed/error).'),
    _('processed'))

KEYWORD_LAST_TRANSACTION_AMOUNT = CurrencyKeyword(
    'last_transaction_amount',
    _('Amount of the last transaction placed for this customer.'),
    25.00)
...
KEYWORD_LAST_TRANSACTION_STATUS = StringKeyword(
    ...
    _('Status of the last transaction placed for this customer '
      '(not processed/processed/error).'),
    _('processed'))
...
class EventSpec(object):
    ...
    def register(cls, event_spec):
        ...
        if event_spec.event_type in cls._event_specs:   # pragma nocoverage
            raise TypeError(
                "EventSpec %r has already been registered" % (
    ...
    def render(self, obj, template):
        ...
        for key, value in list(env.items()):
            text = text.replace(u'{' + str(key) + u'}', str(value))
        return text
