...
class EventTest(SparkMeterTestCaseBase):
    ...
    def test_customer_low_balance_ps_487(self, operator_role):
        ...
        for i in range(5):
            sr = ReadingFactory(meter=str(m.code), kilowatt_hours=1, energy=41 + i)
            self.session.commit()
...
class SMSMessageTest(SparkMeterTestCaseBase):
    def test_get_outgoing(self):
        ...
        # Two-Way, with reply,
        message3 = SMSMessageFactory(text="CMD", direction=SMSMessage.DIRECTION_OUT)
        SMSMessage.create_outgoing(message3.phone_number, "some reply", in_reply_to=message3)
        self.session.commit()
    ...
    def test_maybe_create_alert(self):
        ...
        alert = SMSConfigAlertFactory(event_type=event_type, template="Low on alert {credits_balance}")
        self.session.add(alert.save())
        self.session.commit()
        ...
        assert message.text == "Low on alert 0.00"
    ...
    def test_multiple_customer_with_same_phone_number(self, logger):
        ...
        command = SMSConfigCommandFactory(code="CMD", template="{customer_name}")
        ...
        with freeze_time("2010-01-01 12:01"):
            ...
            assert reply.text == "Customer 1"
        logger.check(("sparkmeter.event.eventdomain", "ERROR", "Multiple customers for phone number +123456"))
        c1 = Customer.query.filter_by(name="Customer 1").one()
        assert c1.phone_number_verified

        c2 = Customer.query.filter_by(name="Customer 2").one()
