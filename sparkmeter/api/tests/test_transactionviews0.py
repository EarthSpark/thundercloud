...
class TransactionAddTest(APIView0TestCaseBase):
    ...
    def _setup_transaction(self):
        self.meter = MeterFactory(serial="SM15R-01-00000001", credit_wallet__value=5, debt_wallet__value=1)
        self.session.commit()
        yield
    ...
    def test_transaction_generic_error(self):
        ...
        with mock.patch.object(Transaction, "create_transactions") as t:
            t.side_effect = TransactionError("generic", "message")
            response = self.post(self.path, data=data)
        self.verify_response(response)
        assert Transaction.query.count() == 0
...
class TransactionViewTest(APIView0TestCaseBase):
    ...
    def _test_sales_account(self, account, account_type):
        ...
        self.verify_response(
            response, variant="view-" + account_type, ignore_values=[str(account.id)], frame=2
        )
    ...
    def test_get_error(self):
        t = TransactionFactory(error="transaction-error", state=Transaction.STATE_ERROR)
        self.session.commit()
        response = self.get(self.path.format(id=t.id))
        self.verify_response(response)
...
class TransactionReverseTest(APIView0TestCaseBase):
    ...
    def _post_and_verify(self, object_id, variant=None, status_code=http.client.CREATED):
    ...
    def test_post_external_id_uuid(self):
        transaction = TransactionFactory(user=self.user, external_id="a6b794be-d1aa-41f3-b920-546260a6068e")
        transaction.from_wallet.value = 100
        transaction.process()
        self.session.commit()
    ...
    def test_post_error_not_found(self):
        self._post_and_verify(
            "7390109f-7103-4777-84f0-89e7deff382a",
    ...
    def test_post_error_pending(self):
        transaction = TransactionFactory(user=self.user, state=Transaction.STATE_PENDING)
        self.session.commit()
        self._post_and_verify(transaction.id, variant="pending", status_code=http.client.UNPROCESSABLE_ENTITY)

    def test_post_error_reversed(self):
        transaction = TransactionFactory(user=self.user, state=Transaction.STATE_REVERSED)
        self.session.commit()
        self._post_and_verify(
            transaction.id, variant="reversed", status_code=http.client.UNPROCESSABLE_ENTITY
        )

    def test_post_error_reversal(self):
        transaction = TransactionFactory(
            user=self.user, state=Transaction.STATE_PROCESSED, origin=Transaction.ORIGIN_REVERSAL
        )
        self.session.commit()
        self._post_and_verify(
            transaction.id, variant="reversal", status_code=http.client.UNPROCESSABLE_ENTITY
        )
    ...
    def test_post_error_already_reversed(self, create):
        ...
        self._post_and_verify(
            transaction.id, variant="already-reversed", status_code=http.client.UNPROCESSABLE_ENTITY
        )
    ...
    def test_post_error_generic(self):
        ...
        with mock.patch.object(Transaction, "reverse") as reverse:
            reverse.side_effect = TransactionError(TransactionError.ERROR_WRONG_TYPE, "unhandled error")
            self._post_and_verify(
                transaction.id, variant="generic-error", status_code=http.client.BAD_REQUEST
            )
