...
from sparkmeter.tests.test_data_factory import (EventFactory, MeterFactory, SMSConfigAlertFactory,
...
class SMSListOutgoingTest(APIView0TestCaseBase):
    ...
    def test_get(self):
        ...
        self.session.commit()
        event1 = EventFactory(event_type=Event.TYPE_CUSTOMER_LOW_BALANCE,
        ...
        response = self.get(self.path, data=json_dumps(data),
        ...
        data = dict(mark_delivered=False)
        response = self.get(self.path, data=data,
        ...
        self.verify_response(response,
                             variant='mark-delivered',
                             ignore_values=[str(message1.id)])
...
class SMSAddIncomingTest(APIView0TestCaseBase):
    ...
    def test_add(self):
        meter = MeterFactory()
        command = SMSConfigCommandFactory(code='CMD', template=u'Reply!')
        self.session.add(command.save())
        self.session.commit()

        data = {
            'id': u'31337 gw s0ftwáre!-876786',
        ...
        assert message.text == u'CMD message'
        ...
        assert message.external_id == u'31337 gw s0ftwáre!-876786'
    ...
    def test_timestamp_formats(self):
        ...
        for i, (fmt, timestamp) in enumerate([
            # Timezone aware (UTC)
            ('2016-01-01T12:34:56.123456789T-0000',
             datetime.datetime(2016, 1, 1, 12, 34, 56, 123456)),
            # Timezone aware (IST)
            ('2016-01-01T12:34:56.123456789T-0545',
             datetime.datetime(2016, 1, 1, 6, 49, 56, 123456)),
            # Timezone unaware
            ('2016-01-01T12:34:56.123456789T',
             datetime.datetime(2016, 1, 1, 12, 34, 56, 123456)),
            # Timezone unaware, microsecond unaware
            ('2016-01-01T12:34:56',
             datetime.datetime(2016, 1, 1, 12, 34, 56, 0)),
            ...
            message = SMSMessage.query.filter_by(direction=SMSMessage.DIRECTION_IN,
                                                 external_id=str(i)).one()
    ...
    def test_verify_customer(self):
        ...
        assert reply
        assert reply.phone_number == '+18008000001'
        msg = u'Thank you! This phone number has been added to str\xebet in SparkMeter.'
    ...
    def test_wrong_code(self):
        ...
        assert reply
        assert reply.phone_number == '+18008000001'
        assert reply.text == u'This SMS code is not recognized by SparkMeter.'
        assert reply.processed
    ...
    def test_number_not_recognized(self):
        ...
        assert reply.text == u'This phone number is not recognized by SparkMeter.'
        assert reply.processed
        assert reply.ground is None
    ...
    def test_message_already_exists(self):
        SMSMessageFactory(external_id=u'external-id')
        self.session.commit()
        data = {
            'id': u'external-id',
            'phone_number': '+18008374966',
