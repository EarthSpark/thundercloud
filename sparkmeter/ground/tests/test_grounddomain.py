...
from sparkmeter.tests.test_data_factory import (GroundFactory, MeterFactory, SalesAccountFactory,
                                                TariffFactory)
...
class GroundTest(SparkMeterTestCaseBase):
    ...
    def test_create_empty_config_defaults(self, config):
        config.update(SERIAL='env-serial',
                      GROUND_NAME='env-name',
                      SPARKCLOUD_API_KEY='env-key')
    ...
    def test_create_duplicate(self):
        ...
        assert str(ctx.value) == u'A ground with name test micr\xf8grid 1 already exists'
        with pytest.raises(ValueError) as ctx:
            Ground.create_empty(self.session, serial=self.ground.serial)
        assert str(ctx.value) == u'A ground with serial groundserial1 already exists'
    ...
    def test_get_last_sync_date(self, config, mocker):
        get_heartbeat_time = mocker.patch(
            'sparkmeter.database.symmetricdsdomain.NodeHost.get_heartbeat_time')
    ...
    def test_remove(self):
        ...
        summary = DashboardDailyTariffSummary(ground=ground, tariff=tariff,
