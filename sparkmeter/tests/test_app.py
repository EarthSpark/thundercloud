...
class HardenSessionCookieTest:
    ...
    def test_secure_preset_true_preserved(self):
        ...
        stub = SimpleNamespace(config={"SESSION_COOKIE_SECURE": True}, _should_use_https=lambda: False)
        SparkmeterApplication._harden_session_cookie(stub)
        assert stub.config["SESSION_COOKIE_SECURE"] is True
...
class AppTest(WebViewTestCaseBase):
    ...
    def test_gzip_passthrough(self, client):
        data = "x" * 1024
        response = client.get(
            url_for("unittest.gzip_test", data=data, mime="application/json", direct="1"),
            headers={"Accept-Encoding": "gzip"},
        )
        assert "Content-Encoding" not in response.headers
    ...
    def test_gzip_too_small(self, client):
        data = "small"
        response = client.get(url_for("unittest.gzip_test", data=data, mime="application/json"))
        assert "Content-Encoding" not in response.headers
        assert response.data == data.encode()
    ...
    def test_gzip_no_user_agent_support(self, client):
        data = "x" * 1024
        response = client.get(
            url_for("unittest.gzip_test", data=data, mime="application/json"), headers={"Accept-Encoding": ""}
        )
        assert "Content-Encoding" not in response.headers
        assert response.data == data.encode()
    ...
    def test_maybe_send_broadcast(self, config, logger, mocker):
        ...
        config["HEROKU"] = False
        assert not app._maybe_send_broadcast()
        logger.check(("sparkmeter.app", "INFO", "No ground found, not sending set-config to active meters."))
        logger.clear()
        assert update_all_active_customer_meters.mock_calls == []
        ...
        logger.check(
            ("sparkmeter.app", "INFO", "Sending set-config to active meters, with override enabled.")
        )
        logger.clear()
        assert update_all_active_customer_meters.mock_calls == [
            mock.call(),
