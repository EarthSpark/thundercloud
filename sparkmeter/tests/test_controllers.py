...
from sparkmeter.tests.test_data_factory import (MeterFactory, SalesAccountFactory,
                                                TotalizerMeterFactory, TransactionFactory)
...
class ControllerTest(SparkMeterTestCaseBase):
    ...
    def test_process_negative_transaction(self, config):
        ...
        message = ('Sending side does not contain enough funds (30.00) to complete '
                   'transfer of value 80.00.')
    ...
    def test_add_totalizer_reading(self, config, send_set_config):
        ...
        meter = TotalizerMeterFactory(
            system_info__last_energy=10,
            system_info__last_energy_datetime=datetime(2013, 1, 1, 1, 0, 1))
        self.session.commit()
    ...
    @pytest.mark.parametrize('reading_delay,transaction_delay,balance', (
        (0.0, 0.0, 1040.0),
        (1.0, 3.0, 963.74625),
        (3.0, 1.0, 1040.0),
    ))
    def test_concurrent_transaction_and_reading_threaded(self,
                                                         config,
                                                         send_set_config,
                                                         session_manager,
                                                         reading_delay,
        ...
        try:
            ...
            tx_thread = threading.Thread(
                ...
                args=(tx.id, transaction_delay, session_manager, tx_exc))
            reading_thread = threading.Thread(
                target=do_reading,
                args=(meter, reading_delay, session_manager, reading_exc))
            tx_thread.start()
