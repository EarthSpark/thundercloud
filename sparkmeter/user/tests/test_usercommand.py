...
class TestManageCommands(SparkMeterTestCaseBase):
    def test_create_user_vendor(self, cli, scoped_session):
        with LogCapture("sparkmeter.user.usercommand") as logger:
            ...
            result = cli(
                "user", "create", "-e", "test@sparkmeter.io", "-p", "pass", "-u", "testuser", "-r", "vendor"
            )
    ...
    def test_create_user_operator(self, cli, scoped_session):
        with LogCapture("sparkmeter.user.usercommand") as logger:
            with mock.patch("sparkmeter.controller.session_scope", scoped_session):
                create_default_roles()
            result = cli(
                "user", "create", "-e", "test@sparkmeter.io", "-p", "pass", "-u", "testuser", "-r", "operator"
            )
            assert result.exit_code == 0
    ...
    def test_create_api_user_vendor(self, cli, scoped_session):
        with LogCapture("sparkmeter.user.usercommand") as logger:
            ...
            with mock.patch("sparkmeter.controller.session_scope", scoped_session):
                create_default_roles()
            result = cli("user", "create", "-u", "api-user", "-r", "api", "-a", account.name)
            assert result.exit_code == 0
    ...
    def test_merge_user(self, cli, scoped_session):
        ...
        usera = UserFactory(
            username="user-a", password="password-a", grounds=[self.ground], accounts=[accounta]
        )
        userb = UserFactory(
            username="user-b", password="password-b", grounds=[self.ground], accounts=[accountb]
        )
        ...
        with mock.patch("sparkmeter.user.usercommand.prompt_bool") as prompt_bool:
            ...
            result = cli("user", "merge", "-a", str(usera.id), "-b", str(userb.id))
            assert result.exit_code == 0
    ...
    def test_merge_user_errors(self, cli, scoped_session):
        with LogCapture("sparkmeter.user.usercommand") as logger:
            ...
            result = cli("user", "merge", "-a", does_not_exist, "-b", does_not_exist_either, "-y")
            assert result.exit_code == 1
            logger.check(
                (
                    "sparkmeter.user.usercommand",
            ...
            result = cli("user", "merge", "-a", str(usera.id), "-b", does_not_exist_either, "-y")
            assert result.exit_code == 1
            logger.check(
                (
                    "sparkmeter.user.usercommand",
            ...
            logger.check(
                ("sparkmeter.user.usercommand", "WARNING", "0 transactions are associated with user user-b"),
