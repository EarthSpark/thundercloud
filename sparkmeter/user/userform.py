...
class BaseUserForm(BaseForm):
    ...
    #: Username
    username = StringField(_("Username"), [DataRequired(_("You must enter a username."))])
    ...
    def validate_username(self, field):
        ...
        if not User.is_username_unique(field.data):
            raise ValidationError(_("This username is already used, please enter another."))
...
class UserAddForm(BaseUserForm):
    ...
    #: User email
    email = StringField(_("Email"), [DataRequired(_("You must enter an email."))])
    ...
    confirm = PasswordField(_("Repeat password"), [DataRequired(_("Password(s) cannot be empty."))])

    def validate_email(self, field):
        """Validate the email field of this form."""
        if not User.is_email_unique(field.data):
            raise ValidationError(_("This email is already used, please enter another."))

    def notification_message(self, user):
        """Build a message to be displayed when a user is added."""
        link = build_link(url_for("user.view", username=user.username), user.username)
