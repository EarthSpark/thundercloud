...
def create(email=None, password=None, username=None, role=None, account=None):
    ...
    while user is None:
        try:
            ...
            first_run = False
            created, user = User.get_one_or_create(session=sql.session,
                                                   id=as_uuid(data['username']),
...
def merge(usera_id, userb_id, force=False):
    ...
    logger.warning("%d transactions are associated with user %s",
                   transactions.count(), userb.username)
