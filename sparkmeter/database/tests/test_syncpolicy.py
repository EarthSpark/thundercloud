...
from sparkmeter.database.symmetricdsdomain import (Channel, Conflict, Node, NodeGroup,
                                                   NodeGroupLink, NodeHost, NodeIdentity, Router,
...
from sparkmeter.meter.meterdomain import (Address, Customer, Meter, MeterBilling, MeterConfig,
                                          MetersTags, MeterSystemInfo, MeterTag, SparkmacNode)
...
class SyncViewTest(SparkMeterTestCaseBase):
    trigger_external_selects = {
        ...
        Customer: [
            'SELECT ground.serial AS ground_serial',
            'FROM ground, meter',
            'WHERE meter.id = cast($(curTriggerValue)."meter_id" as uuid) '
            'AND meter.ground_id = ground.id'
        ],
        MeterBilling: [
            'SELECT ground.serial AS ground_serial',
            'FROM ground, meter',
        ...
        RolesUsers: [
            'SELECT ground.serial AS ground_serial',
            'FROM ground, "user"',
            'WHERE "user".id = cast($(curTriggerValue)."user_id" as uuid) '
            'AND "user".ground_id = ground.id',
        ],
        SalesAccountsUsers: [
            'SELECT ground.serial AS ground_serial',
    ...
    def importSchema(self):
        tables = [NodeGroup, Node, NodeHost, NodeIdentity, NodeGroupLink, Channel,
                  Trigger, Conflict, Router, TriggerRouter]
        sql.metadata.drop_all(
            bind=sql.engine,
    ...
    def test_default_policy(self):
        ...
        links = self.session.query(NodeGroupLink).order_by(
            NodeGroupLink.source_node_group_id).all()
        assert len(links) == 2
        ...
        bundles = [
            ...
            ('tariff', 30, [Tariff]),
            ('meter', 40, [MeterTag, Meter, Customer, MeterBilling,
                           MeterConfig, MeterSystemInfo, SparkmacNode,
                           MetersTags]),
                ...
                for i, table in enumerate(tables):
                    ...
                    if source == 'cloud':
                        if table == Wallet:
                            ...
                            router_type = Router.TYPE_SUBSELECT
                        elif table not in [Ground, MeterTag, Role, RolesUsers,
                                           SMSConfig, Tariff, TransactionSource, User, SalesAccountsUsers,
                                           SystemState, SystemVersion]:
                    ...
                    trigger_router = self.session.query(TriggerRouter).filter_by(
                        trigger_id=trigger_id,
                        router_id=router_id).scalar()
                    if trigger_router is None:
                        self.fail("TriggerRouter for %s+%s does not exist" % (trigger_id,
                                                                              router_id))
    ...
    def test_get_heartbeat_time(self):
        ...
        node_host = NodeHost(node_id='test-node', host_name='hostname',
                             create_time=datetime.datetime.now(),
                             last_restart_time=datetime.datetime.now(),
                             heartbeat_time=datetime.datetime(2009, 1, 1))
        self.session.add(node_host)
        node_host = NodeHost(node_id='test-node', host_name='another-hostname',
                             create_time=datetime.datetime.now(),
                             last_restart_time=datetime.datetime.now(),
