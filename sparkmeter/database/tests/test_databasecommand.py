...
from sparkmeter.database.databasecommand import (clean_tables, cloud_finish_merge,
                                                 cloud_start_merge, force_table_reload)
...
from sparkmeter.tests.test_data_factory import (EventFactory, GroundFactory, MeterFactory,
                                                ...
                                                SMSMessageFactory, UserFactory)
from sparkmeter.transaction.transactiondomain import Transaction, TransactionSource, Wallet


@pytest.fixture()
def logger():
    with LogCapture(('sparkmeter.database.database',
                     'sparkmeter.database.databasecommand')) as logger:
        yield logger


@pytest.fixture()
def getUtility(mocker):
    yield mocker.patch('sparkmeter.database.databasecommand.getUtility',
...
def force_table_reload_mock(mocker):
    yield mocker.patch('sparkmeter.database.sync.force_table_reload',
                       autospec=True, spec_set=True)


@pytest.fixture()
def resetdb(mocker):
    yield mocker.patch('sparkmeter.controller.resetdb',
                       autospec=True, spec_set=True)
...
class DatabaseCommandTest(SparkMeterTestCaseBase):
    ...
    def test_reset(self, cli, resetdb, getUtility):
        ...
        assert resetdb.mock_calls == [
            mock.call(resetschema=True, force=False, empty=False)
        ]
    ...
    def test_reset_demo(self, cli, config, DemoExamples, resetdb, getUtility):
        ...
        cli('database', 'reset-demo',
            '--serial', 'serial',
            '--sparkcloud-api-key', 'secret-key')
        assert getUtility.mock_calls == [
            mock.call(IApplication),
            mock.call().setup_databases(),
        ]
        assert resetdb.mock_calls == [
            mock.call(force=True, resetschema=True)
        ]
        assert DemoExamples.mock_calls == [
            mock.call(mock.ANY),
            mock.call().create_ground(secret_key='secret-key',
                                      serial='serial',
                                      name=None),
    ...
    def test_init_sync(self, cli, create_default_policy, config, getUtility):
        ...
        assert create_default_policy.mock_calls == [
            mock.call(mock.ANY, external_id='serial')
        ]
    ...
    def test_init_sync_external_id(self, cli, create_default_policy, getUtility):
        ...
        assert create_default_policy.mock_calls == [
            mock.call(mock.ANY, external_id='external_id')
        ]
    ...
    def test_cloud_merge(self):
        for cls in [NodeGroup, Node, NodeIdentity, NodeGroupLink, Channel,
                    Trigger, Conflict, Router, TriggerRouter]:
        ...
        trigger = Trigger(
            trigger_id='cloud-test-trigger', source_table_name='user',
            channel=channel,
        ...
        assert list(self.session.execute(text(
            'SELECT COUNT(*) FROM information_schema.constraint_table_usage '
            'WHERE constraint_name = \'transactions_reference_id_fkey\';'))) == [(0,)]
        assert list(self.session.execute(text(
            'SELECT COUNT(*) FROM information_schema.constraint_table_usage '
            'WHERE constraint_name = \'sms_message_in_reply_to_id_fkey\';'))) == [(0,)]
        ...
        assert list(self.session.execute(text(
            'SELECT COUNT(*) FROM information_schema.constraint_table_usage '
            'WHERE constraint_name = \'transactions_reference_id_fkey\';'))) == [(1,)]
        assert list(self.session.execute(text(
            'SELECT COUNT(*) FROM information_schema.constraint_table_usage '
            'WHERE constraint_name = \'sms_message_in_reply_to_id_fkey\';'))) == [(1,)]
        assert self.session.query(Conflict).count() != 0
