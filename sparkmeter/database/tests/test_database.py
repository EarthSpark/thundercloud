...
from sparkmeter.database.database import (bootstrap_production, database_exists,
                                          wait_for_ground_host, wait_for_postgres, wait_for_table,
                                          wait_for_table_count, wait_for_triggers)
...
class DatabaseTest(SparkMeterTestCaseBase):
    ...
    def test_wait_for_table(self, sleep, logger):
        ...
        logger.check(
            ('sparkmeter.database.database',
             'INFO',
             'Waiting for table table_name to be present in schema'),
    ...
    def test_wait_for_table_count(self, wait_for_table, sleep, logger):
        ...
        assert conn.execute.call_count == 2
        assert conn.close.call_count == 2
        assert wait_for_table.mock_calls == [
            mock.call(engine, 'table_name')
        ]
        assert sleep.mock_calls == [
            mock.call(2),
        ]
        ...
        logger.check(
            ...
            ('sparkmeter.database.database',
             'INFO',
             'Table table_name contains 5 items, continuing')
        )
    ...
    def test_bootstrap_production_ground(self,
                                         app,
                                         config,
                                         logger,
                                         resetdb,
        ...
        logger.check(
            ...
            ('sparkmeter.database.database',
             'INFO',
             'No ground found in this database'),
            ('sparkmeter.database.database',
             'WARNING',
             (
    ...
    def test_bootstrap_production_ground_demo(self,
                                              app,
                                              config,
                                              logger,
                                              resetdb,
        ...
        try:
            ...
            logger.check(
                ('sparkmeter.database.database', 'INFO', 'Resetting demo')
            )
        finally:
            environ.stop()

    def test_bootstrap_production_ground_demo_two_grounds(self,
                                                          app,
                                                          config,
                                                          database_has_table,
                                                          logger,
    ...
    def test_bootstrap_production_cloud(self,
                                        mocker,
        ...
        environ = mock.patch.dict(os.environ, dict(
            EXTERNAL_ID='external-id',
        ))
        ...
        try:
            ...
            logger.check(
                ('sparkmeter.database.database',
                 'INFO',
                 'Attempting schema upgrade'),
                ('sparkmeter.database.database',
                 'INFO',
                 'Finished upgrading schema'),
                ('sparkmeter.database.database',
                 'INFO',
                 'No ground found in this database'),
                ('sparkmeter.database.database',
    ...
    def test_bootstrap_production_cloud_init(self,
                                             mocker,
                                             create_default_policy,
                                             wait_for_ground_host,
                                             wait_for_triggers,
        ...
        try:
            ...
            assert wait_for_ground_host.mock_calls == []
            assert wait_for_triggers.mock_calls == [
                mock.call(engine)
            ]
            logger.check(
                ('sparkmeter.database.database',
                ...
                ('sparkmeter.database.database',
                 'INFO',
                 'No admin user found in this database'),
                ('sparkmeter.database.database',
                 'INFO',
    ...
    def test_bootstrap_production_cloud_init_no_ground_or_user(self,
                                                               mocker,
                                                               create_default_policy,
                                                               wait_for_ground_host,
                                                               wait_for_triggers,
