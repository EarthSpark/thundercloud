...
class Wallet(BaseDomain):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "meter_id IS NOT NULL OR sales_account_id IS NOT NULL", name="wallet_references_not_null"
        ),
        # one of meter/ground/sales_account needs to be unset
        CheckConstraint("meter_id IS NULL OR sales_account_id IS NULL", name="wallet_references_one_null"),
        # Can only be one wallet of each type per meter/ground/sales_account
        UniqueConstraint("meter_id", "sales_account_id", "wallet_type", name="wallet_type_unique"),
    )
    ...
    meter_id = Column(UUIDType(binary=False), nullable=True)
...
class Transaction(BaseDomain):
    ...
    from_wallet_id = Column(UUIDType(binary=False), ForeignKey("wallet.id"), info={"label": _("From")})

    #: Wallet which is receiving (to) the transaction
    to_wallet_id = Column(UUIDType(binary=False), ForeignKey("wallet.id"), info={"label": _("To")})
    ...
    def get_by_tariff_period(cls, tariff, ground, start, end):
        ...
        query = (
            cls.query.filter(or_(from_tariff.c.id == tariff.id, to_tariff.c.id == tariff.id))
            .filter(and_(cls.created >= start, cls.created < end))
            .filter(or_(cls.state == Transaction.STATE_PROCESSED, cls.state == Transaction.STATE_REVERSED))
            .with_entities(
            ...
            .outerjoin(
                from_meter,
                and_(from_meter.c.id == from_wallet.c.meter_id, from_meter.c.ground_id == ground.id),
            )
            ...
            .outerjoin(
                to_meter, and_(to_meter.c.id == to_wallet.c.meter_id, to_meter.c.ground_id == ground.id)
            )
        ...
        ).group_by(from_tariff.c.id, to_tariff.c.id)
        return query
    ...
    def get_processed_by_day(cls, ground, start, end, created_before=None):
        ...
        query = (
            (
                ...
                .filter(
                    or_(cls.state == Transaction.STATE_PROCESSED, cls.state == Transaction.STATE_REVERSED)
                )
                .filter(cls.created < created_before)
                .filter(
                    or_(
                        and_(cls.processed_timestamp >= start, cls.processed_timestamp < end),
                        and_(cls.reversed_timestamp >= start, cls.reversed_timestamp < end),
                    )
                )
            )
    ...
    def create_transactions(
        ...
        if not isinstance(from_object, (Meter, SalesAccount)):  # pragma: nocover
            raise TypeError("from_object must be a meter, ground or sales account, not %r" % (from_object,))
        if not isinstance(to_object, (Meter, SalesAccount)):  # pragma: nocover
            raise TypeError("to_object must a meter, ground or sales account, not %r" % (to_object,))
        if amount == 0:
        ...
        if from_wallet.value - amount < 0 and not from_wallet.negative_permitted:
            raise TransactionError(
                ...
                _(
                    (
                        "%(obj)s does not have enough %(wallet_type)s (%(val).2f) "
                        "to cover a transaction of %(amt).2f"
                    ),
        ...
        if external_id is not None:
            if Transaction.get_by_external_id(external_id):
                raise TransactionError(
                    ...
                    _(
                        "A transaction with external_id %(external_id)s already exists.",
    ...
    def has_been_reversed(self):
        ...
        query = self.query.filter_by(
            reference_id=self.id, origin=Transaction.ORIGIN_REVERSAL, state=Transaction.STATE_PROCESSED
        )
    ...
    def process(self):
        ...
        if not self.is_pending():
            raise TransactionError(
                ...
                "Error processing transaction {id}: already processed".format(id=self.id),
            )
        ...
        if self.acct_type not in Wallet.TYPES:
            raise TransactionError(
                ...
                "Error processing transaction {id}: unknown transaction type ({acct_type.code})".format(
                    acct_type=self.acct_type, id=self.id
                ),
            )
        ...
        if self.origin == Transaction.ORIGIN_REVERSAL and self.reference_id:
            ...
            if parent.has_been_reversed():
                raise TransactionError(
                    TransactionError.ERROR_ALREADY_REVERSED, _("Parent transaction already reversed.")
                )
            parent.state = Transaction.STATE_REVERSED
            parent.reversed_timestamp = datetime.datetime.utcnow()
            self.session.add(parent)
            event = Event.create(Event.TYPE_REVERSAL_TRANSACTION, obj=parent)
        ...
        if not self.from_wallet.negative_permitted and self.from_wallet.value - self.amount < 0:
            raise TransactionError(
                TransactionError.ERROR_NOT_ENOUGH_FUNDS,
                _(
                    "Sending side does not contain enough funds ({val:.2f}) "
                    "to complete transfer of value {amount:.2f}."
                ).format(val=self.from_wallet.value, amount=self.amount),
            )
        ...
        msg = (
            "Successfully processed transaction {id}: "
            "from wallet:{account_type}: {starting:.2f} - {amount:.2f} = {ending:.2f}"
        )
        logger.info(
            msg.format(
                starting=from_wallet_starting,
    ...
    def reverse(self, user):
        ...
        if self.has_been_reversed():
            raise TransactionError(TransactionError.ERROR_ALREADY_REVERSED, "Already reversed")
        elif not self.is_processed():
            raise TransactionError(TransactionError.ERROR_NOT_PROCESSED, "Not processed")
...
class TransactionView(BaseView):
    ...
    from_sales_account = relationship("SalesAccount", foreign_keys=[from_sales_account_id])  # type: SalesAccount

    #: sales account receiving this transaction or None
    to_sales_account = relationship("SalesAccount", foreign_keys=[to_sales_account_id])  # type: SalesAccount
    ...
    def get_transaction_view(
        ...
        if sales_account is not None:
            q = q.filter(or_(cls.from_sales_account == sales_account, cls.to_sales_account == sales_account))
        ...
        if query_string:
            q = q.filter(
                or_(
                    text("{}.{}::text ~* :query_string".format(cls.__table__, column.key)).params(
