...
class ReadingGenerator(object):
    ...
    def run_cycle_loop(self):
        ...
        heartbeat_start = now.replace(
            microsecond=0, second=0, minute=(old_div(now.minute, self.cycle_length)) * self.cycle_length
        )

        while True:
            heartbeat_end = heartbeat_start + relativedelta(minutes=self.cycle_length)
            logging.info("* heartbeat period %s TO %s" % (heartbeat_start, heartbeat_end))
            meters = self.get_meters(all_meters)
            self.heartbeat(meters, start=heartbeat_start, end=heartbeat_end)
                ...
                with session_scope(), DisableLogger():
                    for meter in meters[i]:
                        n_readings += self.create_reading(
                            meter, heartbeat_length=interval, heartbeat_start=start, heartbeat_end=end
                        )

            delta = time.time() - t
            sleep_period = int(old_div(float(self.cycle_length * 60), interval))
            logger.info("Added %d readings in %ds (sleeping %ds)" % (n_readings, delta, sleep_period - delta))
            time.sleep(sleep_period)

    def create_reading(self, meter, heartbeat_length=15, heartbeat_start=None, heartbeat_end=None):
        """
        ...
        if meter.system_info.reading is None:
            ...
            last_datetime = now.replace(
                microsecond=0, second=0, minute=(old_div(now.minute, heartbeat_length)) * heartbeat_length
            )
        else:
        ...
        cycles_per_hour = old_div(60.0, self.cycle_length)
        consumption_in_kwh = old_div(self.energy_watts, (cycles_per_hour * 1000.0))
