...
def insert_sales_accounts_users(conn, user_id, sales_account_id):
    ...
    logging.info("- Adding SalesAccount<>User mapping %s, %s" % (user_id, sales_account_id))
    conn.execute(
        sa.sql.text(
            "INSERT INTO sales_accounts_users (id, user_id, sales_account_id)"
...
def insert_sales_account(conn, id, **kwargs):
    ...
    conn.execute(sa.sql.text(stmt), id=sales_account_id, **kwargs)
...
def migrate_users(conn):
    ...
    for result in conn.execute(
        ...
        if n_wallets.fetchone()[0] == 0:
            ...
            conn.execute(sa.sql.text(stmt), wallet_ids=wallet_ids)
            continue
        ...
        insert_sales_accounts_users(conn, user_id=result.id, sales_account_id=sales_account_id)

        # Update the user wallets to point to the sales
        stmt = "UPDATE wallet SET sales_account_id = :id WHERE user_id = :user_id;"
        conn.execute(sa.sql.text(stmt), id=sales_account_id, user_id=result.id)
        logging.info("")
...
def migrate_api_users(conn):
    ...
    for result in conn.execute(
        ...
        insert_sales_accounts_users(conn, user_id=result.id, sales_account_id=result.sales_account_id)
...
def migrate_microgrids(conn):
    ...
    for result in conn.execute("SELECT microgrid.id AS id FROM microgrid"):
        ...
        sales_account_id = insert_sales_account(
            conn, result.id, name="System", markup=0, active=True, system=True, microgrid_id=result.id
        )
        ...
        conn.execute(sa.sql.text(stmt), id=sales_account_id, microgrid_id=result.id)

        # Add operator permissions to the System account
        for operator_id in operator_ids:
            insert_sales_accounts_users(conn, user_id=operator_id, sales_account_id=sales_account_id)
...
def upgrade():
    ...
    op.drop_column("user", "markup")
    op.drop_column("user", "vendor_id")
    op.drop_column("wallet", "user_id")
    op.drop_column("wallet", "microgrid_id")

    op.create_check_constraint(
        "wallet_references_one_null", "wallet", "meter_id IS NULL OR sales_account_id IS NULL"
    )

    op.create_check_constraint(
        "wallet_references_not_null", "wallet", "meter_id IS NOT NULL OR sales_account_id IS NOT NULL"
    )

    op.create_unique_constraint(
        "wallet_type_unique", "wallet", ["meter_id", "sales_account_id", "wallet_type"], schema="public"
    )
