...
def upgrade():
    ...
    op.add_column("event", sa.Column("processed", sa.Boolean(), nullable=False, server_default=sa.false()))
    op.create_unique_constraint("sms_message_external_id_unique", "sms_message", ["external_id"])
    op.add_column(
        "sms_message", sa.Column("processed", sa.Boolean(), nullable=False, server_default=sa.false())
    )


def downgrade():  # pragma: nocoverage
    """Downgrade the database schema from 0.15 to 0.14."""
    op.drop_constraint("sms_message_external_id_unique", "sms_message")
