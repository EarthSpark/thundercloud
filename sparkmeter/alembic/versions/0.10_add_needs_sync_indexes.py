...
def has_index(index):
    ...
    conn = op.get_bind()
    res = conn.execute(
        sql.text("""SELECT COUNT(indexname) FROM pg_indexes WHERE indexname = :index;"""),
        index=index)
...
def upgrade():
    ...
    if not has_index('reading_needs_sync_true'):
        op.create_index('reading_needs_sync_true',
                        'reading', ['needs_sync'], unique=False,
                        postgresql_where=text('needs_sync = TRUE'))
    if not has_index('transaction_needs_sync_true'):
        op.create_index('transaction_needs_sync_true',
                        'transactions', ['needs_sync'], unique=False,
                        postgresql_where=text('needs_sync = TRUE'))
