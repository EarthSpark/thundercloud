...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('template', sa.String(), nullable=False),
        sa.Column('active', sa.Boolean(), nullable=False),
        sa.CheckConstraint(u'code = upper(code)', name='sms_config_command_code_upper'),
        sa.PrimaryKeyConstraint('id'),
    ...
    for code, template in list(SMSConfigCommand.DEFAULT_COMMANDS.items()):
        query = sa.sql.text("INSERT INTO sms_config_command "
    ...
    for message_type, mti in list(SMSConfigMessage.messages.items()):
        query = sa.sql.text("INSERT INTO sms_config_message "
