    ...
    for table, column in TABLES:
        ...
        res = conn.execute(
          ...
          UPDATE %(table)s SET meter_id = meter.id
            FROM meter
           WHERE meter.%(column)s = %(table)s.id"""
        ...
        op.drop_constraint("meter_%s_fkey" % (column,), "meter", type_="foreignkey")
...
def downgrade():  # pragma: nocoverage
    ...
    for table, column in TABLES:
        op.add_column("meter", sa.Column(column, postgresql.UUID(), autoincrement=False, nullable=True))
        conn = op.get_bind()
        res = conn.execute(
            """
          UPDATE meter SET %(column)s = %(table)s.id
            FROM %(table)s
           WHERE %(table)s.meter_id = meter.id"""
            % (dict(table=table, column=column))
        )
        logger.info("Set customer of %r %ss" % (res.rowcount, table))
        op.alter_column("meter", column, nullable=False)

        op.create_foreign_key("meter_%s_fkey" % (column,), "meter", table, [column], ["id"])
