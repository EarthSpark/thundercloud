...
def upgrade():
    ...
    op.add_column(
        "sales_account", sa.Column("global_account", sa.Boolean(), nullable=False, server_default=sa.false())
    )
    op.alter_column("sales_account", "microgrid_id", nullable=True)
    op.alter_column("wallet", "grid_id", nullable=True)
    op.drop_constraint(
        "sales_accounts_users_sales_account_id_fkey", "sales_accounts_users", type_="foreignkey"
    )
    op.drop_constraint("user_api_sales_account_id_fkey", "user", type_="foreignkey")

    conn = op.get_bind()
    sales_account_id = conn.execute("SELECT id FROM sales_account WHERE system = true;").fetchone()[0]
    ...
    for stmt in [
        ('UPDATE "user" SET api_sales_account_id = :new_id WHERE api_sales_account_id = :old_id;'),
        ("UPDATE sales_accounts_users SET sales_account_id = :new_id WHERE sales_account_id = :old_id;"),
    ]:
        conn.execute(
            sa.sql.text(stmt),
    ...
    logger.info("Converted %d no-limit accounts into global sales accounts" % (res.rowcount))
    op.create_foreign_key(
        "sales_accounts_users_sales_account_id_fkey",
    ...
    op.create_foreign_key(
        "user_api_sales_account_id_fkey", "user", "sales_account", ["api_sales_account_id"], ["id"]
    )

    res = conn.execute("UPDATE wallet SET grid_id = NULL WHERE sales_account_id != NULL")
