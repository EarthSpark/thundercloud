...
def upgrade():  # pragma: nocoverage
    ...
    op.create_foreign_key(u'meter_model_id_fkey', 'meter', 'meter_models', ['model_id'], ['id'])
    ...
    if should_populate_models:
        ...
        for meter in meters:
            ...
            try:
                ...
                logger.exception('Could not map meter with ID "%s" and serial "%s" to a model. %s',
                                 meter.id, meter.serial, me)
        ...
        if not config['HEROKU']:  # Never run these commands on the cloud
            ...
            for meter, model, values in meter_updates:
                conn.execute(
                    MeterHelper.update().where(MeterHelper.c.id == meter.id).values(**values)
                )
                logger.info('Mapped meter with serial "%s" to model "%s"',
                            to_change.get('serial', meter.serial), model['name'])
