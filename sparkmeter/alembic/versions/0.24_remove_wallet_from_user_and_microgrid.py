...
def upgrade():
    ...
    op.drop_constraint("microgrid_debt_wallet_id_fkey", "microgrid", type_="foreignkey")
    op.drop_constraint("microgrid_credit_wallet_id_fkey", "microgrid", type_="foreignkey")
    op.drop_constraint("user_debt_wallet_id_fkey", "user", type_="foreignkey")
    op.drop_constraint("user_credit_wallet_id_fkey", "user", type_="foreignkey")
...
def downgrade():  # pragma: nocoverage
    ...
    for table in ["user", "microgrid"]:
        for wallet_type in ["credit", "debt"]:
            ...
            op.add_column(table, sa.Column(column, postgresql.UUID(), autoincrement=False, nullable=True))
