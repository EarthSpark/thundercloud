...
def insert_sales_accounts_users(conn, user_id, sales_account_id):
    ...
    if (
        conn.execute(
            sa.sql.text(
                "SELECT count(*) as count FROM sales_accounts_users "
...
def upgrade():
    ...
    op.add_column(
        "user", sa.Column("account_all_access", sa.Boolean(), nullable=False, server_default=sa.false())
    )
    conn = op.get_bind()
    res = conn.execute(
    ...
    for result in conn.execute("SELECT id AS sales_account_id, name FROM sales_account;"):
        ...
        for user_id, username in users_with_account_all_access_ids:
            if insert_sales_accounts_users(conn, user_id=user_id, sales_account_id=result.sales_account_id):
                logger.info(
                    "Added sales account %r mapping for %r"
                    % (
                        result.name,
                        username,
                    )
