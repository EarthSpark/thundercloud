...
def upgrade():
    ...
    for result in conn.execute(
        ...
        stmt = "DELETE FROM sales_accounts_users WHERE user_id = :user_id"
        conn.execute(sa.sql.text(stmt), user_id=result.user_id)

        stmt = 'UPDATE "user" SET api_sales_account_id = :sales_account_id WHERE "user".id = :user_id'
        conn.execute(sa.sql.text(stmt), sales_account_id=result.sales_account_id, user_id=result.user_id)
...
def downgrade():  # pragma: nocoverage
    ...
    op.drop_column("user", "api_sales_account_id")
