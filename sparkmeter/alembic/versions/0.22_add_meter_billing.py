...
def upgrade():
    ...
    op.create_table('meters_tariffs',
                    sa.Column('id', postgresql.UUID(), nullable=False),
                    sa.Column('last_update', sa.DateTime(), nullable=True),
                    sa.Column('needs_sync', sa.Boolean(), nullable=True),
                    sa.Column('last_sync', sa.DateTime(), nullable=True),
    ...
    op.drop_constraint(u'meter_config_tariff_id_fkey', 'meter_config', type_='foreignkey')
    op.drop_column(u'meter_config', 'tariff_id')
    op.drop_column(u'meter_system_info', 'last_cycle_start')
    op.drop_column(u'meter_system_info', 'total_cycle_energy')
    op.drop_column(u'meter_system_info', 'is_running_plan')
    op.drop_column(u'meter_system_info', 'last_plan_payment_date')
...
def downgrade():  # pragma: nocoverage
    ...
    op.add_column(
        'meter_system_info',
        sa.Column('last_plan_payment_date', postgresql.TIMESTAMP(),
                  autoincrement=False, nullable=True))
    op.add_column(
        'meter_system_info',
        sa.Column('is_running_plan', sa.BOOLEAN(),
                  autoincrement=False, nullable=True))
    op.add_column(
        'meter_system_info',
        sa.Column('total_cycle_energy', postgresql.DOUBLE_PRECISION(precision=53),
                  autoincrement=False, nullable=True))
    op.add_column(u'meter_system_info',
                  sa.Column('last_cycle_start', postgresql.TIMESTAMP(),
                            autoincrement=False, nullable=True))
    op.add_column(u'meter_config',
    ...
    op.create_foreign_key(u'meter_config_tariff_id_fkey', 'meter_config',
                          'tariff', ['tariff_id'], ['id'])
