...
from wtforms.fields import (BooleanField, FloatField, HiddenField, IntegerField, RadioField,
...
class TariffFormBase(BaseForm):
    ...
    load_limit_type = RadioField(_('Load Limit Type'),
                                 default='flat', choices=[('flat', _('Flat')),
                                                          ('scheduled', _('Scheduled'))])

    #: flat load limit, in watts. 4800W is 20A/240V, which is out current hardware capacity
    flat_load_limit = IntegerField(
        _('Load Limit'),
        [NumberRange(min=1, max=4800, message=_('Must be higher than 0'))],
        default=0)
    ...
    tariff_type = RadioField(_('Tariff Type'),
                             default='flat', choices=[('flat', _('Flat rate')),
                                                      ('blockrate', _('Block rate'))])
