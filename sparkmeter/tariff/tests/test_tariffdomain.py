...
class TariffTest(SparkMeterTestCaseBase):
    ...
    def test_get_last_cycle_start_monthly(self):
        ...
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 1, 10, 0, 0)) == datetime.datetime(
            2010, 1, 10, 0, 0
        )
        # Reference date is the same day but later than 00:00: last cycle start is in this month
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 1, 10, 12, 43)) == datetime.datetime(
            2010, 1, 10, 0, 0
        )
        # Reference date is later in the month: last cycle start is in this month
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 1, 17, 8, 23)) == datetime.datetime(
            2010, 1, 10, 0, 0
        )
        # Reference date is earlier in the month: last cycle start is in previous month
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 1, 7, 8, 23)) == datetime.datetime(
            2009, 12, 10, 0, 0
        )

        # Not first month of the year
        # Reference date is exactly the start day datetime: last cycle start is in this month
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 3, 10, 0, 0)) == datetime.datetime(
            2010, 3, 10, 0, 0
        )
        # Reference date is the same day but later than 00:00: last cycle start is in this month
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 3, 10, 12, 43)) == datetime.datetime(
            2010, 3, 10, 0, 0
        )
        # Reference date is later in the month: last cycle start is in this month
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 3, 17, 8, 23)) == datetime.datetime(
            2010, 3, 10, 0, 0
        )
        # Reference date is earlier in the month: last cycle is in previous month
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 3, 7, 8, 23)) == datetime.datetime(
            2010, 2, 10, 0, 0
        )
    ...
    def test_get_last_cycle_start_daily(self):
        ...
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 1, 1, 0, 0)) == datetime.datetime(
            2010, 1, 1, 0, 0
        )
        # After the cycle
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 1, 1, 0, 1)) == datetime.datetime(
            2010, 1, 1, 0, 0
        )
        # Just before the next cycle
        assert tariff.get_last_cycle_start(datetime.datetime(2010, 1, 31, 23, 59)) == datetime.datetime(
            2010, 1, 1, 0, 0
        )
    ...
    def test_get_next_cycle_start_monthly(self):
        ...
        assert tariff.get_next_cycle_start(datetime.datetime(2010, 12, 10, 0, 0)) == datetime.datetime(
            2011, 1, 10, 0, 0
        )
        # Reference date is later in the month: next cycle start is in next month
        assert tariff.get_next_cycle_start(datetime.datetime(2010, 12, 17, 8, 23)) == datetime.datetime(
            2011, 1, 10, 0, 0
        )
        # Reference date is earlier in the month: next cycle start is in this month
        assert tariff.get_next_cycle_start(datetime.datetime(2010, 12, 7, 8, 23)) == datetime.datetime(
            2010, 12, 10, 0, 0
        )

        # Not last month of the year
        # Reference date is exactly the change datetime: next cycle start is in next month
        assert tariff.get_next_cycle_start(datetime.datetime(2010, 3, 10, 0, 0)) == datetime.datetime(
            2010, 4, 10, 0, 0
        )
        # Reference date is later in the month: next cycle start is in next month
        assert tariff.get_next_cycle_start(datetime.datetime(2010, 3, 17, 8, 23)) == datetime.datetime(
            2010, 4, 10, 0, 0
        )
        # Reference date is earlier in the month: next cycle start is in same month
        assert tariff.get_next_cycle_start(datetime.datetime(2010, 3, 7, 8, 23)) == datetime.datetime(
            2010, 3, 10, 0, 0
        )
    ...
    def test_get_next_cycle_start_daily(self):
        ...
        assert tariff.get_next_cycle_start(datetime.datetime(2010, 12, 31, 0, 0)) == datetime.datetime(
            2011, 1, 1, 0, 0
        )
        # Reference date is at an arbitrary point in the middle of the day
        assert tariff.get_next_cycle_start(datetime.datetime(2010, 12, 24, 14, 00)) == datetime.datetime(
            2010, 12, 25, 14, 00
        )
        # Reference date is the minute before the next day
        assert tariff.get_next_cycle_start(datetime.datetime(2010, 12, 30, 23, 59)) == datetime.datetime(
            2010, 12, 31, 23, 59
        )
        # Reference day is Feb 28 on a leap year
        assert tariff.get_next_cycle_start(datetime.datetime(2020, 2, 28, 0, 0)) == datetime.datetime(
            2020, 2, 29, 0, 0
        )
        # Reference day is Feb 29 on a leap year
        assert tariff.get_next_cycle_start(datetime.datetime(2020, 2, 29, 0, 0)) == datetime.datetime(
            2020, 3, 1, 0, 0
        )
...
class TariffBlockrateTest(SparkMeterTestCaseBase):
    ...
    def test_bad_value_type(self):
        for value in [None, "invalid", "invalid", True, False, [], {}]:
            with pytest.raises(TypeError):
...
class TariffTOUTest(SparkMeterTestCaseBase):
    ...
    def test_superset_of(self):
        ...
        rv = tou.superset_of(
            datetime.datetime(2015, 1, 1, 9, tzinfo=tzlocal()),
            datetime.datetime(2015, 1, 1, 10, tzinfo=tzlocal()),
        )
        assert rv

        rv = tou.superset_of(
            datetime.datetime(2015, 1, 1, 7, tzinfo=tzlocal()),
    ...
    def test_superset_of_midnight_crossing(self):
        ...
        rv = tou.superset_of(
            datetime.datetime(2015, 1, 1, 23, tzinfo=tzlocal()),
    ...
    def test_superset_of_errors(self):
        ...
        msg = (
            r"heartbeat_start \(2015-01-01 00:00:00\+00:00\) and "
