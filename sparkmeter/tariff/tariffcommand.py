...
def list_tariffs():
    ...
    for tariff in Tariff.query.order_by(Tariff.name, Tariff.id):
        ...
        logger.info(fmt % (tariff.id, tariff.name, tariff.flat_load_limit, tariff.plan_price,
                           tariff.tariff_type, tariff.display_rate(), tariff.display_tou(), meter_count))
...
def merge(tariff_id_a, tariff_id_b, force=False):
    ...
    logger.warning("%d meters are associated with tariff %s",
                   tariffb_meters.count(), tariff_b.name)

    tariffb_dashboards = DashboardDailyTariffSummary.query.filter_by(tariff_id=tariff_b.id)
    logger.warning("%d dashboard summaries are associated with tariff %s",
                   tariffb_dashboards.count(), tariff_b.name)
