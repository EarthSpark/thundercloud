...
class BaseSalesAccountForm(BaseForm):
    ...
    name = StringField(_('Name'),
                       [DataRequired(_("You must enter a name."))])
    ...
    markup = FloatField(
        ...
        validators=[NumberRange(
            min=0.0,
            message=_("This field cannot be negative, it must be 0.0 or higher."))])
    ...
    def validate_name(self, field):
        ...
        if not SalesAccount.is_name_unique(field.data, skip=self.account):
            raise ValidationError(
                _('This name is already used, please enter another.'))
...
class SalesAccountAddForm(BaseSalesAccountForm):
    ...
    def notification_message(self, sales_account):
        ...
        link = build_link(url_for('sales_account.view',
