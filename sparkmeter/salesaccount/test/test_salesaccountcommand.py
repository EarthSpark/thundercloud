...
def logger():
    ...
    with LogCapture(
        "sparkmeter.salesaccount.salesaccountcommand", "sparkmeter.salesaccount.salesaccountdomain"
    ) as logger:
        yield logger
...
class SalesAccountCommandTest(SparkMeterTestCaseBase):
    def test_list(self, cli, logger):
        ...
        logger.check(
            ...
            (
                "sparkmeter.salesaccount.salesaccountcommand",
                "INFO",
                "0000000a-0001-0000-0000-000000000001 |        SalesAccount1 |                usera",
            ),
            (
                "sparkmeter.salesaccount.salesaccountcommand",
                "INFO",
                "0000000a-0001-0000-0000-000000000002 |        SalesAccount2 |                usera",
            ),
            (
                "sparkmeter.salesaccount.salesaccountcommand",
                "INFO",
                "0000000a-0001-0000-0000-000000000003 |        SalesAccount3 |                userb",
            ),
            (
                "sparkmeter.salesaccount.salesaccountcommand",
                "INFO",
                "0000000a-0001-0000-0000-000000000004 |        SalesAccount4 |                userb",
            ),
        )
    ...
    def test_delete(self, cli, logger):
        ...
        logger.check(
            ("sparkmeter.salesaccount.salesaccountcommand", "INFO", "sales account delete aborted"),
        )
        logger.clear()
        ...
        logger.check(
            ("sparkmeter.salesaccount.salesaccountcommand", "INFO", "sales account global was deleted"),
        )
    ...
    def test_merge(self, cli, logger, api_role):
        ...
        api_user = UserFactory(
            username="api-with-account", roles=[api_role], api_sales_account=global_b, accounts=[global_b]
        )
        self.session.commit()
        ...
        result = cli("salesaccount", "merge", "-a", str(global_a.id), "-b", str(global_b.id), "-y")
        assert result.exit_code == 0
        ...
        with mock.patch("sparkmeter.salesaccount.salesaccountcommand.prompt_bool") as prompt_bool:
            prompt_bool.return_value = False
            result = cli("salesaccount", "merge", "-a", str(global_a.id), "-b", str(global_c.id))
            assert result.exit_code == 1
        ...
        logger.check(
            (
                "sparkmeter.salesaccount.salesaccountcommand",
                "WARNING",
                ("2 wallets associated with sales account 0000000a-0001-0000-0000-000000000003"),
            ),
            (
                "sparkmeter.salesaccount.salesaccountcommand",
                "WARNING",
            ...
            (
                "sparkmeter.salesaccount.salesaccountcommand",
                "WARNING",
                ("0 transactions associated with sales account 0000000a-0001-0000-0000-000000000003"),
            ),
            ("sparkmeter.salesaccount.salesaccountcommand", "INFO", "sales account merge aborted"),
        )
    ...
    def test_merge_errors(self, cli, logger):
        ...
        result = cli("salesaccount", "merge", "-a", str(global_a.id), "-b", str(global_a.id), "-y")
        assert result.exit_code == 1
        logger.check(
            (
                "sparkmeter.salesaccount.salesaccountcommand",
        ...
        result = cli("salesaccount", "merge", "-a", does_not_exist, "-b", does_not_exist_either, "-y")
        assert result.exit_code == 1
        logger.check(
            (
                "sparkmeter.salesaccount.salesaccountcommand",
        ...
        result = cli("salesaccount", "merge", "-a", str(global_a.id), "-b", does_not_exist_either, "-y")
        assert result.exit_code == 1
        logger.check(
            (
                "sparkmeter.salesaccount.salesaccountcommand",
        ...
        result = cli("salesaccount", "merge", "-a", str(global_a.id), "-b", str(restricted_a.id), "-y")
        assert result.exit_code == 1
        msg = (
            "sales account sales åccöünt 1 with id "
            "(0000000a-0000-0000-0000-000000000001) is a restricted sales account"
        )
        logger.check(
            ("sparkmeter.salesaccount.salesaccountcommand", "ERROR", msg),
        )
        logger.clear()

        result = cli("salesaccount", "merge", "-a", str(restricted_a.id), "-b", str(global_a.id), "-y")
        assert result.exit_code == 1
        msg = (
            "sales account sales åccöünt 1 with id "
            "(0000000a-0000-0000-0000-000000000001) is a restricted sales account"
