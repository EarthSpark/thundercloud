...
class SalesAccountViewTest(WebViewTestCaseBase):
    ...
    def test_add_global(self, client):
        ...
        data = {"name": "Global Sales Account", "active": True, "save_button": "Save"}
        response = client.post(path, data=data)
        self.verify_response(response)
    ...
    def test_add_with_user_account_all_access(self, client):
        ...
        assert sorted([a.id for a in vendor2.accounts]) == sorted([account.id, self.system_sales_account.id])

    def test_add_missing_name(self, client):
        path = "/sales-account/add/restricted"
        data = {"ground": self.ground.id, "save_button": "Save"}
        response = client.post(path, data=data)
    ...
    def test_add_duplicated_name(self, client):
        ...
        self.session.commit()
        data = {"name": account.name, "ground": self.ground.id, "save_button": "Save"}
        response = client.post(path, data=data)
        self.verify_response(response, ignore_values=[str(self.system_sales_account.id)])
    ...
    def test_edit_global(self, client):
        ...
        self.verify_response(response, variant="get", ignore_values=[str(self.system_sales_account.id)])

        response = client.get(path % (account.id,), follow_redirects=True)
        assert response.status_code == http.client.OK

        data = {"name": "Global Sales Account #1", "active": True, "save_button": "Save"}
        response = client.post(path % (account.id,), data=data)
        self.verify_response(response, variant="post")

        account = self.session.query(SalesAccount).filter_by(name="Global Sales Account #1").one()
        assert account.active
    ...
    def test_edit_restricted(self, client):
        ...
        data = {"name": "Sales Account #1", "markup": "0.38", "active": True, "save_button": "Save"}
        response = client.post(path % (account.id,), data=data)
        account = self.session.query(SalesAccount).filter_by(name="Sales Account #1").one()
        self.verify_response(response, ignore_values=[str(self.system_sales_account.id)], variant="post")
        assert account.markup == 0.38
        assert account.active
    ...
    def _test_sales_account_pages(self, client, where, user, system):
        for page, tmpl in [
            ...
            for account_type in ["restricted", "global"]:
                ...
                self.verify_response(response, variant=variant, ignore_values=[str(system.id)], frame=2)

    def test_sales_accounts(self, client, config, operator_role, vendor_role):
        other = GroundFactory()
        self.session.commit()
        sr1 = SalesAccountFactory(name="Restricted Ground #1", ground=self.ground)
        sr2 = SalesAccountFactory(name="Restricted Ground #2", ground=other)
        sg = GlobalSalesAccountFactory(name="Global")
        users = [
            OperatorFactory(roles=[operator_role], username="operator-none", accounts=[], grounds=[]),
            OperatorFactory(
                roles=[operator_role], username="operator-only-global", accounts=[sg], grounds=[other]
            ),
            OperatorFactory(
                roles=[operator_role], username="operator-only-1", accounts=[sr1], grounds=[self.ground]
            ),
            OperatorFactory(
                roles=[operator_role], username="operator-only-2", accounts=[sr2], grounds=[other]
            ),
            OperatorFactory(
                roles=[operator_role],
            ...
            VendorFactory(roles=[vendor_role], username="vendor-none", accounts=[], grounds=[]),
            VendorFactory(roles=[vendor_role], username="vendor-only-global", accounts=[sg], grounds=[other]),
            VendorFactory(
                roles=[vendor_role], username="vendor-only-1", accounts=[sr1], grounds=[self.ground]
            ),
            VendorFactory(roles=[vendor_role], username="vendor-only-2", accounts=[sr2], grounds=[other]),
            VendorFactory(
                roles=[vendor_role],
                username="vendor-all",
                accounts=[sr1, sr2, sg],
    ...
    def test_api_sales_accounts(self, client, config, api_role, operator_role):
        ...
        operator = OperatorFactory(
            roles=[operator_role], username="operator-only-global", accounts=[sg], grounds=[self.ground]
        )
        api_users = [
            UserFactory(roles=[api_role], username="api-with-account", api_sales_account=sg),
            UserFactory(roles=[api_role], username="api"),
        ]
        self.session.commit()
        system = SalesAccount.get_system()
        config["HEROKU"] = True
    ...
    def test_sales_accounts_bad_account_type(self, client):
        path = "/sales-account/bad-account-type.json"
