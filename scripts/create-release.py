...
def update_version(version_info):
    ...
    path = os.path.join(os.path.dirname(__file__), "..", "sparkmeter", "__version__.py")
    with open(path, "rt") as f:
        data = f.read()
    for name, value in zip(["major", "minor", "patch"], version_info):
        data = re.sub(r"{} = \d+".format(name), "{} = {}".format(name, value), data)
...
def git_commit(version_info):
    ...
    subprocess.check_call(["git", "commit", "-m", version_info.message, "sparkmeter/__version__.py"])


def git_tag(version_info):
    """Tag the commit."""
    logger.info("Creating git tag")
    subprocess.check_call(["git", "tag", "-am", version_info.message, version_info.tag_name])
...
def git_push(branch, version_info):
    ...
    subprocess.check_call(["git", "push", "origin", "{0}:refs/tags/{0}".format(version_info.tag_name)])

    logger.info("Pushing HEAD to {}, submitting directly, skipping review".format(branch))
    subprocess.check_call(["git", "push", "origin", "HEAD:refs/for/{}%submit".format(branch)])
