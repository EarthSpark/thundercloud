...
var _exportData = function ( dt, inOpts )
{
	...
	var header;
	// double-header is currently only used by tariff-list, does not support
    // non-visible columns
	if (dt.table().header().getAttribute('class') === 'double-header') {
        header = $(dt.table().node())
                .find( 'thead tr:first th' )
                .map( function(_, i) {
                         return strip(i.innerHTML);
                } );
    } else {
